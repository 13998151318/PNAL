...
def train(Model,Criterion,Optimizer,Train_gene,train_ill,test_ill, entid2data, ent2data_with_desc, entid_1, entid_2):
    ...
    for epoch in range(EPOCH_NUM):
        ...
        if ACTUALLY_DO_TRAINING:
            candidate_dict = generate_candidate_dict(Model,train_ent1s,train_ent2s,for_candidate_ent1s,
                                                        for_candidate_ent2s,ent2data_with_desc,Train_gene.index2entity)
            Train_gene.train_index_gene(candidate_dict) #generate training data with candidate_dict

            #train
            epoch_loss,epoch_train_time = ent_align_train(Model,Criterion,Optimizer,Train_gene,ent2data_with_desc)
        Optimizer.zero_grad()
        torch.cuda.empty_cache()
        if ACTUALLY_DO_TRAINING:
...
def zero_train(Model,Criterion,Optimizer,Train_gene,train_ill,test_ill,entid2data, entid_1, entid_2): 
    save(Model,train_ill,test_ill,entid2data,EPOCH_NUM-1)
    # test(Model,train_ill,entid2data,TEST_BATCH_SIZE,context="EVAL IN TRAIN SET")
    test(Model, test_ill, entid2data, TEST_BATCH_SIZE, context="EVAL IN TEST SET:")
