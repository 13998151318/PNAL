...
def read_data(data_path = DATA_PATH,des_dict_path = DES_DICT_PATH):
    ...
    def read_id2object(file_paths):
        ...
        return id2object
    def read_id2object_with_trans(ent_ids_1, ent_ids_2, translated):
        id2object = {}
        ent_1_ids = []
        with open(ent_ids_1, 'r', encoding='utf-8') as f:
            print('loading a (id2object)file...  ' + ent_ids_1)
            for line in f:
                th = line.strip('\n').split('\t')
                ent_1_ids.append(int(th[0]))
                #id2object[int(th[0])] = th[1]
        with open(translated, 'r', encoding='utf-8') as f:
            print('loading a (id2object)file...  ' + translated)
            line_num = -1
            for line in f:
                line_num += 1
                th = line.strip('\n').replace(" ", "_")
                id2object[ent_1_ids[line_num]] = th
    ...
    if WITH_TRANS:
        index2entity = read_id2object_with_trans(data_path + "ent_ids_1", data_path + "ent_ids_2", data_path + "translated_google.txt")
        before_trans_index2entity = read_id2object([data_path + "ent_ids_1",data_path + "ent_ids_2"])
    else:
        index2entity = read_id2object([data_path + "ent_ids_1",data_path + "ent_ids_2"])
        before_trans_index2entity = index2entity
    index2rel = read_id2object([data_path + "rel_ids_1",data_path + "rel_ids_2"])
    entity2index = {e:idx for idx,e in index2entity.items()}
    before_trans_entity2index = {e:idx for idx,e in before_trans_index2entity.items()}
    ...
    if USE_DESC and des_dict_path!= None:
    ...
    ent2tokenids = ent2Tokens_gene(Tokenizer,None,entids,index2entity)
    ent2data = ent2bert_input(entids,Tokenizer,ent2tokenids)

    ent2tokenids_with_desc = ent2Tokens_gene(Tokenizer,ent2desTokens,entids,index2entity)
    ent2data_with_desc = ent2bert_input(entids,Tokenizer,ent2tokenids_with_desc)

    return ent_ill, train_ill, test_ill, index2rel, index2entity, rel2index, entity2index, ent2data, ent2data_with_desc, rel_triples_1, rel_triples_2, entid_1, entid_2
