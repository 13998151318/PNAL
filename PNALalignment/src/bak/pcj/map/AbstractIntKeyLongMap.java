 ...
 *  maps from int values to long values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractIntKeyLongMap implements IntKeyLongMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractIntKeyLongMap() { }
    ...
    public void putAll(IntKeyLongMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyLongMap))
            return false;
        IntKeyLongMap map = (IntKeyLongMap)obj;
        if (size() != map.size())
            return false;
        IntKeyLongMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultLongHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
