 ...
 *  maps from char values to float values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractCharKeyFloatMap implements CharKeyFloatMap {
    ...
    protected AbstractCharKeyFloatMap() { }
    ...
    public void putAll(CharKeyFloatMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof CharKeyFloatMap))
            return false;
        CharKeyFloatMap map = (CharKeyFloatMap)obj;
        if (size() != map.size())
            return false;
        CharKeyFloatMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultCharHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultFloatHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
