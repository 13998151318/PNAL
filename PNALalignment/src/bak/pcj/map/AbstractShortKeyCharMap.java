 ...
 *  maps from short values to char values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractShortKeyCharMap implements ShortKeyCharMap {
    ...
    protected AbstractShortKeyCharMap() { }
    ...
    public void putAll(ShortKeyCharMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ShortKeyCharMap))
            return false;
        ShortKeyCharMap map = (ShortKeyCharMap)obj;
        if (size() != map.size())
            return false;
        ShortKeyCharMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultShortHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultCharHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
