 ...
 *  maps from char values to long values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractCharKeyLongMap implements CharKeyLongMap {
    ...
    protected AbstractCharKeyLongMap() { }
    ...
    public void putAll(CharKeyLongMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof CharKeyLongMap))
            return false;
        CharKeyLongMap map = (CharKeyLongMap)obj;
        if (size() != map.size())
            return false;
        CharKeyLongMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultCharHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultLongHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
