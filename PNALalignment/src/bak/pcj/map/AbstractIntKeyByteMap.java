 ...
 *  maps from int values to byte values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractIntKeyByteMap implements IntKeyByteMap {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyByteMap))
            return false;
        IntKeyByteMap map = (IntKeyByteMap)obj;
        if (size() != map.size())
            return false;
        IntKeyByteMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultByteHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
