 ...
 *  maps from byte values to int values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractByteKeyIntMap implements ByteKeyIntMap {
    ...
    protected AbstractByteKeyIntMap() { }
    ...
    public void putAll(ByteKeyIntMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ByteKeyIntMap))
            return false;
        ByteKeyIntMap map = (ByteKeyIntMap)obj;
        if (size() != map.size())
            return false;
        ByteKeyIntMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultByteHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultIntHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
