 ...
 *  maps from int values to boolean values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractIntKeyBooleanMap implements IntKeyBooleanMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractIntKeyBooleanMap() { }
    ...
    public void putAll(IntKeyBooleanMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyBooleanMap))
            return false;
        IntKeyBooleanMap map = (IntKeyBooleanMap)obj;
        if (size() != map.size())
            return false;
        IntKeyBooleanMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultBooleanHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
