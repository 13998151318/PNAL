 ...
 *  maps from long values to int values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractLongKeyIntMap implements LongKeyIntMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractLongKeyIntMap() { }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof LongKeyIntMap))
            return false;
        LongKeyIntMap map = (LongKeyIntMap)obj;
        if (size() != map.size())
            return false;
        LongKeyIntMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultLongHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultIntHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
