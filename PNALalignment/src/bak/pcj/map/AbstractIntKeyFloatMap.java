 ...
 *  maps from int values to float values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractIntKeyFloatMap implements IntKeyFloatMap {
    ...
    protected AbstractIntKeyFloatMap() { }
    ...
    public void putAll(IntKeyFloatMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyFloatMap))
            return false;
        IntKeyFloatMap map = (IntKeyFloatMap)obj;
        if (size() != map.size())
            return false;
        IntKeyFloatMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultFloatHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
