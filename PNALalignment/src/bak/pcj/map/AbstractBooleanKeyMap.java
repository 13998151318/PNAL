 ...
 *  maps from boolean values to objects. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractBooleanKeyMap implements BooleanKeyMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractBooleanKeyMap() { }
    ...
    public void putAll(BooleanKeyMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof BooleanKeyMap))
            return false;
        BooleanKeyMap map = (BooleanKeyMap)obj;
        if (size() != map.size())
            return false;
        BooleanKeyMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultBooleanHashFunction.INSTANCE.hash(i.getKey()) ^ i.getValue().hashCode());
        }
        return h;
    }
