 ...
 *  maps from float values to short values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractFloatKeyShortMap implements FloatKeyShortMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractFloatKeyShortMap() { }
    ...
    public void putAll(FloatKeyShortMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof FloatKeyShortMap))
            return false;
        FloatKeyShortMap map = (FloatKeyShortMap)obj;
        if (size() != map.size())
            return false;
        FloatKeyShortMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultFloatHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultShortHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
