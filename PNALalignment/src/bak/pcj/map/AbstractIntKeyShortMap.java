 ...
 *  maps from int values to short values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractIntKeyShortMap implements IntKeyShortMap {
    ...
    protected AbstractIntKeyShortMap() { }
    ...
    public void putAll(IntKeyShortMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyShortMap))
            return false;
        IntKeyShortMap map = (IntKeyShortMap)obj;
        if (size() != map.size())
            return false;
        IntKeyShortMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultShortHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
