 ...
 *  maps from int values to char values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractIntKeyCharMap implements IntKeyCharMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractIntKeyCharMap() { }
    ...
    public void putAll(IntKeyCharMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntKeyCharMap))
            return false;
        IntKeyCharMap map = (IntKeyCharMap)obj;
        if (size() != map.size())
            return false;
        IntKeyCharMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultIntHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultCharHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
