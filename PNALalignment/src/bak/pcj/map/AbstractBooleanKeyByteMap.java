 ...
 *  maps from boolean values to byte values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractBooleanKeyByteMap implements BooleanKeyByteMap {
    ...
    protected AbstractBooleanKeyByteMap() { }

    public void clear() {
        BooleanKeyByteMapIterator i = entries();
        while (i.hasNext()) {
            i.next();
            i.remove();
        }
    ...
    public void putAll(BooleanKeyByteMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof BooleanKeyByteMap))
            return false;
        BooleanKeyByteMap map = (BooleanKeyByteMap)obj;
        if (size() != map.size())
            return false;
        BooleanKeyByteMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultBooleanHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultByteHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
