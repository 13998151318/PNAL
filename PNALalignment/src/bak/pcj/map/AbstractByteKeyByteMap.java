 ...
 *  maps from byte values to byte values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractByteKeyByteMap implements ByteKeyByteMap {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ByteKeyByteMap))
            return false;
        ByteKeyByteMap map = (ByteKeyByteMap)obj;
        if (size() != map.size())
            return false;
        ByteKeyByteMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultByteHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultByteHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
