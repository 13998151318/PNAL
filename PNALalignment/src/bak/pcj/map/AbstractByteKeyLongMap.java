 ...
 *  maps from byte values to long values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractByteKeyLongMap implements ByteKeyLongMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractByteKeyLongMap() { }
    ...
    public void putAll(ByteKeyLongMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ByteKeyLongMap))
            return false;
        ByteKeyLongMap map = (ByteKeyLongMap)obj;
        if (size() != map.size())
            return false;
        ByteKeyLongMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultByteHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultLongHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
