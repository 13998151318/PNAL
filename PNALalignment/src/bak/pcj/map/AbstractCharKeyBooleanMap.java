 ...
 *  maps from char values to boolean values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractCharKeyBooleanMap implements CharKeyBooleanMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractCharKeyBooleanMap() { }
    ...
    public void putAll(CharKeyBooleanMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof CharKeyBooleanMap))
            return false;
        CharKeyBooleanMap map = (CharKeyBooleanMap)obj;
        if (size() != map.size())
            return false;
        CharKeyBooleanMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultCharHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultBooleanHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
