 ...
 *  maps from float values to double values. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractFloatKeyDoubleMap implements FloatKeyDoubleMap {

    /** Default constructor to be invoked by sub-classes. */
    protected AbstractFloatKeyDoubleMap() { }
    ...
    public void putAll(FloatKeyDoubleMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof FloatKeyDoubleMap))
            return false;
        FloatKeyDoubleMap map = (FloatKeyDoubleMap)obj;
        if (size() != map.size())
            return false;
        FloatKeyDoubleMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultFloatHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultDoubleHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
