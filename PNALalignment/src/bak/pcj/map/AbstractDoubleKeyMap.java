 ...
 *  maps from double values to objects. All operations that can be implemented
 *  using iterators
 ...
 *  @since      1.0
 */
public abstract class AbstractDoubleKeyMap implements DoubleKeyMap {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof DoubleKeyMap))
            return false;
        DoubleKeyMap map = (DoubleKeyMap)obj;
        if (size() != map.size())
            return false;
        DoubleKeyMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultDoubleHashFunction.INSTANCE.hash(i.getKey()) ^ i.getValue().hashCode());
        }
        return h;
    }
