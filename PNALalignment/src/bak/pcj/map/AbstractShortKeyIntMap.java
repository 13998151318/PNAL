 ...
 *  maps from short values to int values. All operations that can be implemented
 *  using iterators
...
public abstract class AbstractShortKeyIntMap implements ShortKeyIntMap {
    ...
    protected AbstractShortKeyIntMap() { }
    ...
    public void putAll(ShortKeyIntMap map) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ShortKeyIntMap))
            return false;
        ShortKeyIntMap map = (ShortKeyIntMap)obj;
        if (size() != map.size())
            return false;
        ShortKeyIntMapIterator i = entries();
        while (i.hasNext()) {
    ...
    public int hashCode() {
        ...
        while (i.hasNext()) {
            i.next();
            h += (DefaultShortHashFunction.INSTANCE.hash(i.getKey()) ^ DefaultIntHashFunction.INSTANCE.hash(i.getValue()));
        }
        return h;
    }
