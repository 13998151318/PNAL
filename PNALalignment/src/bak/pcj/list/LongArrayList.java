 ...
 *  @since      1.0
 */
public class LongArrayList extends AbstractLongList implements Cloneable, Serializable {
    ...
    private LongArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public LongArrayList(int capacity, double growthFactor) {
     ...
     */
    public LongArrayList(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            LongArrayList c = (LongArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultLongHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
