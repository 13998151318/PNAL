 ...
 *  @since      1.0
 */
public class FloatArrayList extends AbstractFloatList implements Cloneable, Serializable {
    ...
    private FloatArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public FloatArrayList(int capacity, double growthFactor) {
     ...
     */
    public FloatArrayList(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            FloatArrayList c = (FloatArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultFloatHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
