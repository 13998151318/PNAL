...
public class IntArrayDeque extends AbstractIntList implements IntDeque, Cloneable, Serializable {
    ...
    private IntArrayDeque(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public IntArrayDeque(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            IntArrayDeque c = (IntArrayDeque)super.clone();
    ...
    // ---------------------------------------------------------------
    //      Operations declared by IntDeque
    // ---------------------------------------------------------------
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof IntDeque))
            return false;
        IntDeque s = (IntDeque)obj;
        if (size != s.size())
            return false;
        IntListIterator i2 = s.listIterator();
        for (int i = 0, idx = first; i < size; i++) {
            if (data[idx] != i2.next())
                return false;
            if (++idx == data.length)
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultIntHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
