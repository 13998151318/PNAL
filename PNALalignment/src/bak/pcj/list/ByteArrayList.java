 ...
 *  @since      1.0
 */
public class ByteArrayList extends AbstractByteList implements Cloneable, Serializable {
    ...
    private ByteArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public ByteArrayList(int capacity, double growthFactor) {
     ...
     */
    public ByteArrayList(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            ByteArrayList c = (ByteArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultByteHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
