 ...
 *  @since      1.0
 */
public class CharArrayList extends AbstractCharList implements Cloneable, Serializable {
    ...
    private CharArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public CharArrayList(int capacity, int growthChunk) {
    ...
    public int ensureCapacity(int capacity) {
        if (capacity > data.length) {
            char[] newdata = new char[capacity = computeCapacity(capacity)];
    ...
    public Object clone() {
        try {
            CharArrayList c = (CharArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultCharHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
