 ...
 *  @since      1.0
 */
public class BooleanArrayDeque extends AbstractBooleanList implements BooleanDeque, Cloneable, Serializable {
    ...
    private BooleanArrayDeque(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     *
     *  @see        #BooleanArrayDeque(int,double)
     */
     ...
     *  @since      1.1
     */
    public BooleanArrayDeque(boolean[] a) {
        this(a.length);
     ...
     */
    public BooleanArrayDeque(int capacity) {
     ...
     */
    public BooleanArrayDeque(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            BooleanArrayDeque c = (BooleanArrayDeque)super.clone();
    ...
    // ---------------------------------------------------------------
    //      Operations declared by BooleanDeque
    // ---------------------------------------------------------------
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof BooleanDeque))
            return false;
        BooleanDeque s = (BooleanDeque)obj;
        if (size != s.size())
            return false;
        BooleanListIterator i2 = s.listIterator();
        for (int i = 0, idx = first; i < size; i++) {
            if (data[idx] != i2.next())
                return false;
            if (++idx == data.length)
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultBooleanHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
