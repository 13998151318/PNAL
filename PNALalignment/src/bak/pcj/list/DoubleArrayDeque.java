...
public class DoubleArrayDeque extends AbstractDoubleList implements DoubleDeque, Cloneable, Serializable {
    ...
    private DoubleArrayDeque(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     *
     *  @see        #DoubleArrayDeque(int,double)
     */
     ...
     *  @since      1.1
     */
    public DoubleArrayDeque(double[] a) {
        this(a.length);
     ...
     */
    public DoubleArrayDeque(int capacity) {
     ...
     */
    public DoubleArrayDeque(int capacity, int growthChunk) {
    ...
    public int ensureCapacity(int capacity) {
        if (capacity > data.length) {
            double[] newdata = new double[capacity = computeCapacity(capacity)];
            toArray(newdata);
            first = 0;
            last = size;
            data = newdata;
    ...
    public Object clone() {
        try {
            DoubleArrayDeque c = (DoubleArrayDeque)super.clone();
    ...
    // ---------------------------------------------------------------
    //      Operations declared by DoubleDeque
    // ---------------------------------------------------------------
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DoubleDeque))
            return false;
        DoubleDeque s = (DoubleDeque)obj;
        if (size != s.size())
            return false;
        DoubleListIterator i2 = s.listIterator();
        for (int i = 0, idx = first; i < size; i++) {
            if (data[idx] != i2.next())
                return false;
            if (++idx == data.length)
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultDoubleHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
