...
public class ShortArrayList extends AbstractShortList implements Cloneable, Serializable {
    ...
    private ShortArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public ShortArrayList(int capacity, double growthFactor) {
     ...
     */
    public ShortArrayList(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            ShortArrayList c = (ShortArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultShortHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
