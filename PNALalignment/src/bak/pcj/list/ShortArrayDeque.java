 ...
 *  @since      1.0
 */
public class ShortArrayDeque extends AbstractShortList implements ShortDeque, Cloneable, Serializable {
    ...
    private ShortArrayDeque(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     *
     *  @see        #ShortArrayDeque(int,double)
     */
     ...
     */
    public ShortArrayDeque(ShortCollection c) {
        this(c.size());
        addAll(c);
    }
     ...
     *  @since      1.1
     */
    public ShortArrayDeque(short[] a) {
        this(a.length);
     ...
     */
    public ShortArrayDeque(int capacity, double growthFactor) {
     ...
     */
    public ShortArrayDeque(int capacity, int growthChunk) {
    ...
    public int ensureCapacity(int capacity) {
        if (capacity > data.length) {
            short[] newdata = new short[capacity = computeCapacity(capacity)];
            toArray(newdata);
            first = 0;
            last = size;
            data = newdata;
    ...
    public Object clone() {
        try {
            ShortArrayDeque c = (ShortArrayDeque)super.clone();
    ...
    // ---------------------------------------------------------------
    //      Operations declared by ShortDeque
    // ---------------------------------------------------------------
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ShortDeque))
            return false;
        ShortDeque s = (ShortDeque)obj;
        if (size != s.size())
            return false;
        ShortListIterator i2 = s.listIterator();
        for (int i = 0, idx = first; i < size; i++) {
            if (data[idx] != i2.next())
                return false;
            if (++idx == data.length)
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultShortHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
