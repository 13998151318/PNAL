 ...
 *  @since      1.0
 */
public class LongArrayDeque extends AbstractLongList implements LongDeque, Cloneable, Serializable {
    ...
    private LongArrayDeque(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public LongArrayDeque(LongCollection c) {
        this(c.size());
        addAll(c);
    }
     ...
     */
    public LongArrayDeque(int capacity, double growthFactor) {
     ...
     */
    public LongArrayDeque(int capacity, int growthChunk) {
    ...
    public int ensureCapacity(int capacity) {
        if (capacity > data.length) {
            long[] newdata = new long[capacity = computeCapacity(capacity)];
            toArray(newdata);
            first = 0;
            last = size;
            data = newdata;
    ...
    public Object clone() {
        try {
            LongArrayDeque c = (LongArrayDeque)super.clone();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof LongDeque))
            return false;
        LongDeque s = (LongDeque)obj;
        if (size != s.size())
            return false;
        LongListIterator i2 = s.listIterator();
        for (int i = 0, idx = first; i < size; i++) {
            if (data[idx] != i2.next())
                return false;
            if (++idx == data.length)
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultLongHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
