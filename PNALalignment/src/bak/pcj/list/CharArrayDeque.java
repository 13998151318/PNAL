 ...
 *  @since      1.0
 */
public class CharArrayDeque extends AbstractCharList implements CharDeque, Cloneable, Serializable {
    ...
    // ---------------------------------------------------------------
    //      Operations declared by CharDeque
    // ---------------------------------------------------------------
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0, idx = first; i < size; i++) {
            h = (int)(31*h + DefaultCharHashFunction.INSTANCE.hash(data[idx]));
            if (++idx == data.length)
                idx = 0;
        }
        return h;
