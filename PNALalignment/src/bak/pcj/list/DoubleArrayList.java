...
public class DoubleArrayList extends AbstractDoubleList implements Cloneable, Serializable {
    ...
    private DoubleArrayList(int capacity, int growthPolicy, double growthFactor, int growthChunk) {
        if (capacity < 0)
     ...
     */
    public DoubleArrayList(int capacity, double growthFactor) {
     ...
     */
    public DoubleArrayList(int capacity, int growthChunk) {
    ...
    public Object clone() {
        try {
            DoubleArrayList c = (DoubleArrayList)super.clone();
    ...
    public int hashCode() {
        int h = 1;
        for (int i = 0; i < size; i++)
            h = (int)(31*h + DefaultDoubleHashFunction.INSTANCE.hash(data[i]));
        return h;
    }
