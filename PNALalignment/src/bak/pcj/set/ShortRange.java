...
public class ShortRange implements Comparable, Serializable {
     ...
     *
     *  @param      first
     *              the first short value in the range.
     *
     *  @param      last
     ...
     */
    /*
    public ShortRange(short first, int length) {
        if (length <= 0)
            throw new IllegalArgumentException();
        //  first+length-1 > MAX_VALUE rewritten to avoid overflow
        if (first-1 > Short.MAX_VALUE-length)
            throw new IllegalArgumentException();
        this.first = first;
        this.last = (short)(first+length-1);
    }
    */
     ...
     *
     *  @return     the first short value in this range.
     */
    public short first()
    { return first; }
     ...
     *
     *  @return     the last short value in this range.
     */
    public short last()
    { return last; }
    ...
    private ShortRange quickMergeWith(ShortRange range) {
        short nfirst = first < range.first ? first : range.first;
        short nlast = last > range.last ? last : range.last;
    ...
    public int intersectionLength(ShortRange range) {
        ...
        if (first >= range.first && first <= range.last) {
            short end = last <= range.last ? last : range.last;
            len = ((int)(end-first))+1;
        } else
     ...
     *              if <tt>obj</tt> is not of class <tt>ShortRange</tt>.
     */
