...
public class LongRange implements Comparable, Serializable {
     ...
     *
     *  @param      first
     *              the first long value in the range.
     *
     *  @param      last
     ...
     */
    /*
    public LongRange(long first, int length) {
        if (length <= 0)
            throw new IllegalArgumentException();
        //  first+length-1 > MAX_VALUE rewritten to avoid overflow
        if (first-1 > Long.MAX_VALUE-length)
            throw new IllegalArgumentException();
        this.first = first;
        this.last = (long)(first+length-1);
    }
    */
     ...
     *
     *  @return     the first long value in this range.
     */
    public long first()
    { return first; }
     ...
     *
     *  @return     the last long value in this range.
     */
    public long last()
    { return last; }
    ...
    private LongRange quickMergeWith(LongRange range) {
        long nfirst = first < range.first ? first : range.first;
        long nlast = last > range.last ? last : range.last;
    ...
    public int intersectionLength(LongRange range) {
        ...
        if (first >= range.first && first <= range.last) {
            long end = last <= range.last ? last : range.last;
            len = ((int)(end-first))+1;
        } else
     ...
     *              if <tt>obj</tt> is not of class <tt>LongRange</tt>.
     */
