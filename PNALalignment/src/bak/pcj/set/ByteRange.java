...
public class ByteRange implements Comparable, Serializable {
     ...
     *
     *  @param      first
     *              the first byte value in the range.
     *
     *  @param      last
     ...
     */
    /*
    public ByteRange(byte first, int length) {
        if (length <= 0)
            throw new IllegalArgumentException();
        //  first+length-1 > MAX_VALUE rewritten to avoid overflow
        if (first-1 > Byte.MAX_VALUE-length)
            throw new IllegalArgumentException();
        this.first = first;
        this.last = (byte)(first+length-1);
    }
    */
     ...
     *
     *  @return     the first byte value in this range.
     */
    public byte first()
    { return first; }
     ...
     *
     *  @return     the last byte value in this range.
     */
    public byte last()
    { return last; }
    ...
    private ByteRange quickMergeWith(ByteRange range) {
        byte nfirst = first < range.first ? first : range.first;
        byte nlast = last > range.last ? last : range.last;
    ...
    public int intersectionLength(ByteRange range) {
        ...
        if (first >= range.first && first <= range.last) {
            byte end = last <= range.last ? last : range.last;
            len = ((int)(end-first))+1;
        } else
     ...
     *              if <tt>obj</tt> is not of class <tt>ByteRange</tt>.
     */
