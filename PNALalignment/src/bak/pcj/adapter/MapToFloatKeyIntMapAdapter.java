...
public class MapToFloatKeyIntMapAdapter extends AbstractFloatKeyIntMap implements FloatKeyIntMap {
     ...
     */
    public MapToFloatKeyIntMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(float key) {
        lastValue = (Integer)map.get(new Float(key));
    ...
    public int get(float key) {
        Integer value = (Integer)map.get(new Float(key));
    ...
    public int put(float key, int value) {
        Integer oldValue = (Integer)map.put(new Float(key), new Integer(value));
        return oldValue == null ? MapDefaults.defaultInt() : oldValue.intValue();
    }

    public int remove(float key) {
        Integer value = (Integer)map.remove(new Float(key));
        return value == null ? MapDefaults.defaultInt() : value.intValue();
    }
    ...
    { return Adapter.isFloatKeyIntAdaptable(map); }
