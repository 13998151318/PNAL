 ...
 *  @since      1.0
 */
public class MapToByteKeyDoubleMapAdapter extends AbstractByteKeyDoubleMap implements ByteKeyDoubleMap {
    ...
    public boolean containsKey(byte key) {
        lastValue = (Double)map.get(new Byte(key));
    ...
    public double get(byte key) {
        Double value = (Double)map.get(new Byte(key));
    ...
    public double put(byte key, double value) {
        Double oldValue = (Double)map.put(new Byte(key), new Double(value));
        return oldValue == null ? MapDefaults.defaultDouble() : oldValue.doubleValue();
    }

    public double remove(byte key) {
        Double value = (Double)map.remove(new Byte(key));
        return value == null ? MapDefaults.defaultDouble() : value.doubleValue();
    }
    ...
    { return Adapter.isByteKeyDoubleAdaptable(map); }
