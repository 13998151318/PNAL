...
import bak.pcj.map.AbstractDoubleKeyByteMap;
 ...
 *  @since      1.0
 */
public class MapToDoubleKeyByteMapAdapter extends AbstractDoubleKeyByteMap implements DoubleKeyByteMap {
     ...
     *              {@link Byte Byte}.
     */
    public MapToDoubleKeyByteMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(double key) {
        lastValue = (Byte)map.get(new Double(key));
    ...
    public byte get(double key) {
        Byte value = (Byte)map.get(new Double(key));
    ...
    public byte put(double key, byte value) {
        Byte oldValue = (Byte)map.put(new Double(key), new Byte(value));
        return oldValue == null ? MapDefaults.defaultByte() : oldValue.byteValue();
    }

    public byte remove(double key) {
        Byte value = (Byte)map.remove(new Double(key));
        return value == null ? MapDefaults.defaultByte() : value.byteValue();
    }
    ...
    { return Adapter.isDoubleKeyByteAdaptable(map); }
