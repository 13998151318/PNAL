 ...
 *  in the client code:
 *  <pre>
 *      MapToIntKeyBooleanMapAdapter s;
 *      ...
...
public class MapToIntKeyBooleanMapAdapter extends AbstractIntKeyBooleanMap implements IntKeyBooleanMap {
     ...
     */
    public MapToIntKeyBooleanMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(int key) {
        lastValue = (Boolean)map.get(new Integer(key));
    ...
    public boolean get(int key) {
        Boolean value = (Boolean)map.get(new Integer(key));
    ...
    public boolean put(int key, boolean value) {
        Boolean oldValue = (Boolean)map.put(new Integer(key), new Boolean(value));
        return oldValue == null ? MapDefaults.defaultBoolean() : oldValue.booleanValue();
    }

    public boolean remove(int key) {
        Boolean value = (Boolean)map.remove(new Integer(key));
        return value == null ? MapDefaults.defaultBoolean() : value.booleanValue();
    }
    ...
    { return Adapter.isIntKeyBooleanAdaptable(map); }
