...
import bak.pcj.map.AbstractObjectKeyFloatMap;
 ...
 *  in the client code:
 *  <pre>
 *      MapToObjectKeyFloatMapAdapter s;
 *      ...
 ...
 *  @since      1.1
 */
public class MapToObjectKeyFloatMapAdapter extends AbstractObjectKeyFloatMap implements ObjectKeyFloatMap {
     ...
     */
    public MapToObjectKeyFloatMapAdapter(Map map) {
        if (map == null)
     ...
     *              {@link Float Float}.
     */
    public MapToObjectKeyFloatMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public float put(Object key, float value) {
        Float oldValue = (Float)map.put(key, new Float(value));
        return oldValue == null ? MapDefaults.defaultFloat() : oldValue.floatValue();
    }

    public float remove(Object key) {
        Float value = (Float)map.remove(key);
        return value == null ? MapDefaults.defaultFloat() : value.floatValue();
    }
     ...
     *  values and only {@link Float Float} values.
     *
     *  @return     <tt>true</tt> if the underlying map is
     *              valid; returns <tt>false</tt> otherwise.
     */
    public boolean validate()
    { return Adapter.isObjectKeyFloatAdaptable(map); }
