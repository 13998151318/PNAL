...
import bak.pcj.map.AbstractByteKeyFloatMap;
 ...
 *  @since      1.0
 */
public class MapToByteKeyFloatMapAdapter extends AbstractByteKeyFloatMap implements ByteKeyFloatMap {
     ...
     */
    public MapToByteKeyFloatMapAdapter(Map map) {
        if (map == null)
     ...
     *              {@link Float Float}.
     */
    public MapToByteKeyFloatMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(byte key) {
        lastValue = (Float)map.get(new Byte(key));
    ...
    public float get(byte key) {
        Float value = (Float)map.get(new Byte(key));
    ...
    public float put(byte key, float value) {
        Float oldValue = (Float)map.put(new Byte(key), new Float(value));
        return oldValue == null ? MapDefaults.defaultFloat() : oldValue.floatValue();
    }

    public float remove(byte key) {
        Float value = (Float)map.remove(new Byte(key));
        return value == null ? MapDefaults.defaultFloat() : value.floatValue();
    }
    ...
    { return Adapter.isByteKeyFloatAdaptable(map); }
