...
public class MapToByteKeyByteMapAdapter extends AbstractByteKeyByteMap implements ByteKeyByteMap {
     ...
     *              {@link Byte Byte}.
     */
    public MapToByteKeyByteMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(byte key) {
        lastValue = (Byte)map.get(new Byte(key));
    ...
    public byte get(byte key) {
        Byte value = (Byte)map.get(new Byte(key));
    ...
    public byte put(byte key, byte value) {
        Byte oldValue = (Byte)map.put(new Byte(key), new Byte(value));
        return oldValue == null ? MapDefaults.defaultByte() : oldValue.byteValue();
    }

    public byte remove(byte key) {
        Byte value = (Byte)map.remove(new Byte(key));
        return value == null ? MapDefaults.defaultByte() : value.byteValue();
    }
    ...
    { return Adapter.isByteKeyByteAdaptable(map); }
