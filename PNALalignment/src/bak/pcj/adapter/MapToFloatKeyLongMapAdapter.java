...
import bak.pcj.map.AbstractFloatKeyLongMap;
 ...
 *  @since      1.0
 */
public class MapToFloatKeyLongMapAdapter extends AbstractFloatKeyLongMap implements FloatKeyLongMap {
     ...
     */
    public MapToFloatKeyLongMapAdapter(Map map) {
        if (map == null)
     ...
     *              {@link Long Long}.
     */
    public MapToFloatKeyLongMapAdapter(Map map, boolean validate) {
        if (map == null)
            Exceptions.nullArgument("map");
        this.map = map;
        lastValue = null;
    ...
    public boolean containsKey(float key) {
        lastValue = (Long)map.get(new Float(key));
    ...
    public long get(float key) {
        Long value = (Long)map.get(new Float(key));
    ...
    public long put(float key, long value) {
        Long oldValue = (Long)map.put(new Float(key), new Long(value));
        return oldValue == null ? MapDefaults.defaultLong() : oldValue.longValue();
    }

    public long remove(float key) {
        Long value = (Long)map.remove(new Float(key));
        return value == null ? MapDefaults.defaultLong() : value.longValue();
    }
    ...
    { return Adapter.isFloatKeyLongAdaptable(map); }
