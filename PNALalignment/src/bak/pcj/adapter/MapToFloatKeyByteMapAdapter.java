 ...
 *  @since      1.0
 */
public class MapToFloatKeyByteMapAdapter extends AbstractFloatKeyByteMap implements FloatKeyByteMap {
    ...
    public boolean containsKey(float key) {
        lastValue = (Byte)map.get(new Float(key));
    ...
    public byte get(float key) {
        Byte value = (Byte)map.get(new Float(key));
    ...
    public byte put(float key, byte value) {
        Byte oldValue = (Byte)map.put(new Float(key), new Byte(value));
        return oldValue == null ? MapDefaults.defaultByte() : oldValue.byteValue();
    }

    public byte remove(float key) {
        Byte value = (Byte)map.remove(new Float(key));
        return value == null ? MapDefaults.defaultByte() : value.byteValue();
    }
    ...
    { return Adapter.isFloatKeyByteAdaptable(map); }
