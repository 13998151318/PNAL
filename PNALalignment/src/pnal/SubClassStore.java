...
public class SubClassStore extends HashSubThingStore<Integer> {
	...
	public String toTsv(SubPair<Integer> p) {
		return fs1.entity(p.sub)+"\t"+fs2.entity(p.supr)+"\t"+p.truthValue+"\n";
	}
	
    @Override
    public void setTruthValue(Integer sub, Integer supr, TruthValue truthValue) {
        set(new SubPair<Integer>(sub, supr, truthValue));
    }
