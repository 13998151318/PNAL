...
public abstract class HashSubThingStore<T> extends SubThingStore<T> implements Closeable {
     ...
     */
    @Override
    public TruthValue getTruthValue(T sub, T supr) {
        SubPair<T> e = get(sub, supr);
        return (e != null ? e.truthValue : new TruthValue());
    }
    ...
    public void set(T sub, T supr, TruthValue val) {
    ...
    public Collection<Pair<Object, TruthValue>> superOfScored(T sub) {
    ...
    public Collection<Pair<Object, TruthValue>> subOfScored(T supr) {
        List<Pair<Object, TruthValue>> result = new ArrayList<Pair<Object, TruthValue>>();
        for (SubPair<T> a : superIndex.getOrEmpty(supr)) {
