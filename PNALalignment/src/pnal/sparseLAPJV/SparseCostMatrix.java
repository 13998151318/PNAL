...
public class SparseCostMatrix
{
	 ...
	 */
	public SparseCostMatrix( final double[] cc, final int[] kk, final int[] number, int nRows, int nCols , int cc_size)
	{
        int column_without_cost = 0;
		this.cc = cc;
		this.kk = kk;
		this.number = number;
		this.nCols = nCols;
		...
		this.cardinality = cc_size;
        //number's last element is the artificial row (additional)
		this.nRows = nRows;
		// loop on each row
		this.start = new int[ nRows + 1 ];
		if ( nRows > 0 )
		{
			start[ 0 ] = 0;
		}
		...
		for ( int j = 0; j < colHistogram.length; j++ )
		{
		    ...
		    if ( colHistogram[ j ] == 0 ) { 
                D.p("colHistogram[ " + j + " ] == 0");
                this.cc[cc_size] = MAX_VALUE;
                this.kk[cc_size] = j;
                cc_size++;
                column_without_cost++;
                this.number[nRows]++;
        ...
        //if (column_without_cost > 0){
        this.nCols++;
        this.nRows++;
        this.cc[cc_size] = 0.;
        this.kk[cc_size] = this.nCols-1;
        cc_size++;
        this.number[this.nRows - 1]++;
        start[ this.nRows-1 ] = start[ this.nRows - 2 ] + this.number[ this.nRows - 2 ];
        D.p("this.nRows:" + this.nRows);
        D.p("start[ this.nRows-1 ]:" + start[ this.nRows-1 ]);
        D.p("this.number[this.nRows - 1]:" + this.number[this.nRows - 1]);
        this.cardinality = cc_size;
        D.p("this.cardinality:" + this.cardinality);
        D.p("column_without_cost:" + column_without_cost); //column_without_cost:51
        //}
	...
	public final SparseCostMatrix vcat( final SparseCostMatrix B )
	{
		...
		return new SparseCostMatrix( cc2, kk2, number2, nRows, nCols, cc2.length );
	}
	...
	public final SparseCostMatrix hcat( final SparseCostMatrix B )
	{
		...
		return new SparseCostMatrix( cc2, kk2, number2, nRows, nCols + B.nCols, cc2.length );
	}
	...
	public final SparseCostMatrix transpose()
	{
		...
		return new SparseCostMatrix( cc2, kk2, number2, nCols, nRows, cc2.length );
	}
